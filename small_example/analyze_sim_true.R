...
err.array[,,new_iter] <- err.mat
err.std.array[,,new_iter] <- err.mat.std
mat.true.phentrajs[,new_iter] <- true.per.time

#Tests
qxtest_mat[new_iter,1:3] <- Qx_test(trajs_neut[time %in% ((0:10)/100),], eff_sizes, perms = 0)
qxtest_mat[new_iter,4] <- Qx_test(trajs_neut[time %in% ((0:10)/100),], eff_sizes, perms = 10000)[3]
qxtest_mat[new_iter,5:7] <- Qx_test(trajs_mom_smoothtime[time %in% ((0:10)/100),], eff_sizes, perms = 0)
qxtest_mat[new_iter,8] <- Qx_test(trajs_mom_smoothtime[time %in% ((0:10)/100),], eff_sizes, perms = 10000)[3]
qxtest_mat[new_iter,9:11] <- Qx_test(trajs_est_wt_l1[time %in% ((0:10)/100),], eff_sizes, perms = 0)
qxtest_mat[new_iter,12] <- Qx_test(trajs_est_wt_l1[time %in% ((0:10)/100),], eff_sizes, perms = 10000)[3]
